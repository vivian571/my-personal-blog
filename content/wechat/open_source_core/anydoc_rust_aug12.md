...
## 手把手教学：如何在 Python/Node.js/终端 中使用 anydoc？

anydoc 提供了原生 Rust CLI、Python 包以及 Node.js/WASM 绑定。
...
```bash
anydoc convert --input ./presentation.pptx --output ./slides.md
```

### 3. 在 Cursor / Claude Code 中作为 Agent 技能调用

只需将 `anydoc` 放入你的工具链中，当你的 Agent 遇到用户上传的各种奇怪附件时，Agent 自动调用 `anydoc` 将其转为标准的 Markdown 并写入上下文！
